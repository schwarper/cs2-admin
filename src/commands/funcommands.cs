...
public partial class Admin : BasePlugin
{
    ...
    public void Command_Beacon(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (value > 0)
        {
            ...
            {
                Timer timer = Instance.AddTimer(3.0f, () => target.Beacon(), TimerFlags.REPEAT | TimerFlags.STOP_ON_MAPCHANGE);

                target.AddTimer(timer, PlayerUtils.PlayerTimerFlags.Beacon);
            }
        }
        else
        {
            foreach (CCSPlayerController target in players)
            {
                target.RemoveTimer(PlayerUtils.PlayerTimerFlags.Beacon);
            }
        }
    ...
    [RequiresPermissions("@css/slay")]
    [CommandHelper(minArgs: 1, "<#userid|name|all @ commands> <time>", whoCanExecute: CommandUsage.CLIENT_AND_SERVER)]
    public void Command_Shake(CCSPlayerController? player, CommandInfo command)
    {
        (List<CCSPlayerController> players, string adminname, string targetname) = Find(player, command, 1, false, true, MultipleFlags.IGNORE_DEAD_PLAYERS);
        ...
        if (players.Count == 1)
        {
            PrintToChatAll("css_shake<player>", adminname, targetname, value);
        }
        else
        {
            PrintToChatAll("css_shake<multiple>", adminname, targetname, value);
        }

        Discord.SendMessage($"[{player?.SteamID ?? 0}] {adminname} -> css_shake <{command.GetArg(1)}> <{value}>");
    }

    [ConsoleCommand("css_unshake")]
    [RequiresPermissions("@css/slay")]
    [CommandHelper(minArgs: 1, "<#userid|name|all @ commands>", whoCanExecute: CommandUsage.CLIENT_AND_SERVER)]
    public void Command_UnShake(CCSPlayerController? player, CommandInfo command)
    {
        (List<CCSPlayerController> players, string adminname, string targetname) = Find(player, command, 1, false, true, MultipleFlags.IGNORE_DEAD_PLAYERS);
        ...
        if (players.Count == 1)
        {
            PrintToChatAll("css_unshake<player>", adminname, targetname);
        }
        else
        {
            PrintToChatAll("css_unshake<multiple>", adminname, targetname);
        }
